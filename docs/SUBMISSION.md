...
```
AI coding agents lose context every session. Perseus resolves workspace state before the agent sees it. Mimir carries memory across sessions. Both target AMD MI300X — MIT licensed, zero cloud lock-in.
```
...
2. **Mimir (Memory Engine)** — Carries architectural decisions, bug fixes, conventions, and insights across sessions. SQLite+FTS5, 23 MCP tools, <5ms recall. Agents remember what happened last Tuesday.

The demo shows a 3-session progression:
- **Session 1 (Cold Start):** Agent discovers project from scratch — 2,184 tokens burned
- **Session 2 (Warm Start):** Perseus + Mimir provide context — 94% token reduction
...
| **Memory Engine** | Mimir (Rust + SQLite+FTS5) | Filesystem on AMD instance storage |
...
| **MCP Protocol** | Model Context Protocol | 45+ combined tools across Perseus + Mimir |
...
5. **ROCm container image** — Single Docker image with Perseus + Mimir + vLLM ROCm pre-configured
...
- **Mimir** — Rust, SQLite+FTS5, 23 MCP tools (MIT)
...
3. Mimir Cross-Session Memory (1:00-1:30)
